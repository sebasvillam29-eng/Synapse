...
const WorkspacePage = () => {
  ...
  const leftWidth = pdfState === "loaded" ? "40%" : "30%";

  return (
    <div className="flex flex-col h-screen">
      {/* Top breadcrumb bar */}
      <div className="shrink-0 px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      {/* Document metadata bar (only when PDF loaded) */}
      {pdfState === "loaded" && (
        <div className="shrink-0 px-6 py-2 text-xs text-muted-foreground text-center" style={{ backgroundColor: "rgba(255,255,255,0.02)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          8 pages &nbsp;·&nbsp; ~2,400 words &nbsp;·&nbsp; Estimated study time: 18 min
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        >
            {pdfState === "empty" && (
              <div className="flex-1 flex items-center justify-center p-12">
                <button
                  ...
                  className="w-full max-w-sm rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-5 py-14 px-12 text-muted-foreground transition-all duration-300 hover:border-primary/60 hover:text-foreground hover:bg-primary/5"
                  style={{ borderColor: "rgba(124,92,191,0.4)" }}
                >
                  <Upload className="w-12 h-12" style={{ color: "hsl(var(--primary) / 0.6)" }} />
                  <div className="text-center space-y-1.5">
                    <p className="text-base font-medium text-foreground">Drop your PDF here</p>
                    <p className="text-[13px] text-muted-foreground">up to 20MB · PDF, Word, or paste text</p>
                  </div>
                </button>
                <div className="absolute bottom-8 left-0 w-full text-center" style={{ width: leftWidth }}>
                  <button className="text-[13px] text-primary/70 hover:text-primary transition-colors underline underline-offset-2">
                    or paste your notes
                  </button>
                </div>
              </div>
            ...
            {pdfState === "loaded" && (
              <div className="flex-1 flex flex-col animate-fade-in">
                <div className="flex items-center justify-between px-6 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                  <div className="flex items-center gap-2 text-sm">
        ...
        {/* RIGHT — AI tools */}
        <div className="flex-1 flex flex-col overflow-hidden pt-6 px-5">
          ...
          {/* Separator + scrollable output */}
          <div className="mt-6 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }} />
          <div className="flex-1 overflow-y-auto pt-6 pb-6 px-1">
            {toolLoading && (
...
const ToolCard = ({ tool, active, onClick }: { tool: typeof toolCards[0]; active: boolean; onClick: () => void }) => {
  const isLastOdd = tool.id === "mindmap";
  return (
    <button
      onClick={onClick}
      className={`rounded-xl border text-left transition-all duration-200 cursor-pointer ${
        isLastOdd ? "col-span-2 max-w-[calc(50%-6px)]" : ""
      } ${
        active
          ? "shadow-[0_0_24px_-6px_hsl(var(--primary)/0.35)]"
      ...
      style={{
        ...
        padding: "20px 24px",
        ...(active
          ? { borderColor: "hsl(var(--primary))", borderWidth: "1.5px", backgroundColor: "rgba(124,92,191,0.12)" }
          : {}),
      }}
    >
      <div className="flex items-center gap-3">
        <tool.icon className={`w-5 h-5 shrink-0 ${active ? "text-primary" : "text-muted-foreground"}`} />
        <p className="text-sm font-medium text-foreground">{tool.title}</p>
      </div>
      <p className="text-[13px] text-muted-foreground mt-1.5">{tool.desc}</p>
    </button>
  );
};
...
const SummaryOutput = () => {
  const sections = [
    { heading: "1. Photosynthesis Overview", body: "Photosynthesis is the process by which green plants convert light energy into chemical energy stored in glucose. It occurs primarily in the chloroplasts of mesophyll cells." },
    { heading: "2. Light-Dependent Reactions", body: "These reactions take place in the thylakoid membranes. Water molecules are split (photolysis), releasing oxygen. ATP and NADPH are produced as energy carriers." },
    { heading: "3. Calvin Cycle", body: "The Calvin Cycle occurs in the stroma. CO₂ is fixed by RuBisCO into G3P, which is used to synthesize glucose. It requires ATP and NADPH from light reactions." },
  ];

  return (
    <div className="rounded-xl border border-border bg-card p-6 animate-fade-in">
      <h3 className="text-base font-bold text-foreground mb-5">AI Summary</h3>
      <div className="space-y-0">
        {sections.map((s, i) => (
          <div
            ...
            className="pl-3"
            style={{
              borderLeft: "1px solid rgba(124,92,191,0.3)",
              marginTop: i === 0 ? 0 : 20,
            }}
          >
            <h4 className="text-sm font-bold text-foreground mb-2">{s.heading}</h4>
            <p className="text-sm leading-[1.8]" style={{ color: "rgba(232,232,240,0.7)" }}>{s.body}</p>
          </div>
        ))}
      </div>
...
const QuizOutput = () => {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 animate-fade-in space-y-4">
