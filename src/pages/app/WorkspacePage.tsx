...
const mapNodes: { x: number; y: number; label: string }[] = [
...
const MindMapContent = () => {
  ...
  const totalNodes = mapNodes.length + 1; // +1 for center
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (count >= totalNodes + mapNodes.length) clearInterval(interval); // nodes + lines
    }, 180);
  ...
  if (step === "pick") {
    return (
      <div className="animate-fade-in" style={{ maxWidth: 520 }}>
        <h1 className="text-foreground mb-1" style={{ fontFamily: "'Instrument Serif', serif", fontSize: 26 }}>Mind Map</h1>
        <p className="text-[14px] text-muted-foreground mb-6">Choose what to map.</p>
          ...
          {topics.map((t, i) => {
            ...
            return (
              <button
                ...
                style={{
                  padding: "14px 18px",
                  background: selected ? "#16162a" : "#111120",
                  border: `1px solid ${selected ? "rgba(124,92,191,0.5)" : "rgba(255,255,255,0.07)"}`,
                  borderRadius: 12,
                  boxShadow: selected ? "0 4px 16px rgba(124,92,191,0.1)" : "none",
                }}
                onMouseEnter={e => { if (selectedIdx !== i) { e.currentTarget.style.borderColor = "rgba(124,92,191,0.3)"; e.currentTarget.style.background = "#141424"; } }}
                onMouseLeave={e => { if (selectedIdx !== i) { e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; e.currentTarget.style.background = "#111120"; } }}
              >
                ...
                <div className="shrink-0 flex items-center justify-center" style={{ width: 20, height: 20 }}>
                  <div style={{
                    ...
                    border: `2px solid ${selected ? "#9d7fe0" : "rgba(232,232,240,0.3)"}`,
                    background: selected ? "#9d7fe0" : "transparent",
                    transition: "all 0.2s",
                  }} />
                </div>
              </button>
        ...
        <button
          ...
          className="w-full text-[14px] font-semibold transition-all duration-250 mt-5"
          style={{
            padding: "13px 24px",
            borderRadius: 10,
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: 580 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-foreground" style={{ fontFamily: "'Instrument Serif', serif", fontSize: 24 }}>{topicLabel}</h1>
      ...
      >
        {/* Lines */}
        {mapNodes.map((b, i) => {
          const lineIdx = totalNodes + i; // lines appear after all nodes
        ...
        <g style={{
          ...
          <text x={centerX} y={centerY + 4} textAnchor="middle" fill="#c4a8ff" fontSize="12" fontWeight="600">{topicLabel.length > 16 ? topicLabel.slice(0, 14) + "…" : topicLabel}</text>
        </g>

        {/* Branch nodes */}
        {mapNodes.map((b, i) => {
          const nodeIdx = i + 1; // center is 0
